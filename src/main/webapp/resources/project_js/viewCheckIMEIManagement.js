...
function gotoPagination(){
	$('div#initialloader').fadeIn('fast');
	var ellipsis = "...";
	var goTopageNo=$('#gotoPageId').val();
	if(goTopageNo== null || goTopageNo==""){
		alert("Please Insert Page Number Then Click Go To Button")
		return false;
	}
	var roleType = $("body").attr("data-roleType");
	...
	var pageNo=goTopageNo;
	var pageSize =info.length;
	var startPage=0;
	if(pageNo>1){
		startPage=(pageNo-1)*pageSize
	}
	alert(startPage+"#"+goTopageNo+"#"+pageSize);
	var featureName=$('#featureName').val();
	if(featureName ==""){
		featureName=null;
	}

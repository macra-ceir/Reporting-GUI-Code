...
@Controller
public class GraphReportsController {
	...
	@RequestMapping(value=
		{"/graphReports"},method={org.springframework.web.bind.annotation.
				RequestMethod.GET,org.springframework.web.bind.annotation.RequestMethod.POST}
			)
	    public ModelAndView viewConfigManagement(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		 log.info(" view Graph Reports entry point."); 
		 mv.setViewName("viewsGhraphReport");
		log.info(" view Graph Reports exit point."); 
		return mv; 
	}
	...
	public ResponseEntity<?> activeDeviceGraphDashboard(@RequestBody Map<?, ?> graphRequest) {
	    try {
	    	 log.info("activeDeviceGraphDashboard Reports request ["+graphRequest.toString()+"]"); 
	    	 System.out.println("activeDeviceGraphDashboard Reports request ["+graphRequest.toString()+"]");
	        Object response =  analyticsFeign.dashboard(graphRequest);
	        String apiResponse = new Gson().toJson(response);
	        
	        log.info("activeDeviceGraphDashboard API Response ["+apiResponse.toString()+"]"); 
	        System.out.println("activeDeviceGraphDashboard API Response ["+apiResponse.toString()+"]");
	        
	        return new ResponseEntity<>(apiResponse, HttpStatus.OK);
	    } catch (Exception e) {
	        e.printStackTrace();
	        System.out.println("Exception activeDeviceGraphDashboard Reports request ["+graphRequest.toString()+"]");
	        return new ResponseEntity<>(Collections.EMPTY_LIST, HttpStatus.SERVICE_UNAVAILABLE);
	    }
	}

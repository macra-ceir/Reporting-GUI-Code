...
public class AllowedTACController {
	...
	@RequestMapping(value=
		{"/allowedTAC"},method={org.springframework.web.bind.annotation.
				RequestMethod.GET,org.springframework.web.bind.annotation.RequestMethod.POST}
			)
	    public ModelAndView viewConfigManagement(HttpSession session) {
		ModelAndView mv = new ModelAndView();
		 log.info(" view AllowedTAC entry point."); 
		 mv.setViewName("viewAllowedTAC");
		log.info(" view AllowedTAC exit point."); 
		return mv; 
	}
	
	
	@PostMapping("/allowedTAC/viewTag") 
	public @ResponseBody ConfigContentModel VipListViewTag (@RequestBody FilterRequest filterRequest,HttpSession session)  {
		...
		ConfigContentModel response= feignCleintImplementation.viewAllowedTacFeign(filterRequest);
		log.info("viplist response from currency api "+response);
		return response;
	}
	
	
	@PutMapping("/allowedTAC/update")
	...
	@PostMapping("exportAllowedTACData")
	@ResponseBody
	public FileExportResponse exportToExcel(@RequestBody FilterRequest filterRequest,HttpSession session)
	{
		...
		response= feignCleintImplementation.allowedTacView(filterRequest, filterRequest.getPageNo(), filterRequest.getPageSize(), file);
